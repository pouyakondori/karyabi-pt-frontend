  ...
  return (
    <div className="space-y-5">
      ...
      {step === 0 ? (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field labelKey="form.portugalRegion">
            <select value={form.portugalRegion} onChange={(event) => setField("portugalRegion", event.target.value as PortugalRegion)}>
              {regions.map((region) => (
      ...
      {step === 1 ? (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field labelKey="form.residencyPermitType">
            <input value={form.residencyPermitType} onChange={(event) => setField("residencyPermitType", event.target.value)} />
          </Field>
      ...
      <div className="flex flex-wrap gap-3">
        {step > 0 ? (
          <button className="rounded-2xl bg-slate-200 px-4 py-3 text-sm font-semibold text-slate-700" onClick={() => setStep(step - 1)} type="button">
            {t("common.previous")}
          </button>
        ) : null}
        {step < steps.length - 1 ? (
          <button className="rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white" onClick={() => setStep(step + 1)} type="button">
            {t("common.next")}
          </button>
        ) : (
          <button
