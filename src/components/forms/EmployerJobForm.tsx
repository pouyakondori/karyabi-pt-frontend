...
const fieldsWithRequiredBadge: FormField[] = ["title", "companyName", "description", "applicationDeadline"];
const validatedFields: FormField[] = ["title", "companyName", "description", "applicationDeadline", "salaryMin", "salaryMax", "vacancies"];
...
function Chevron({ isOpen }: { isOpen: boolean }) {
  return <span className={`text-xs text-slate-400 transition ${isOpen ? "rotate-180" : ""}`}>⌄</span>;
}

function RequiredBadge() {
  return <span className="ms-2 rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-bold text-rose-600">{t("common.required")}</span>;
}
              ...
              {options.map((option) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-right text-sm transition ${
                    option.value === value ? "bg-emerald-50 text-emerald-700" : "text-slate-700 hover:bg-slate-50"
                  }`}
                  onClick={() => {
              ...
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option) => (
                  <button
                    key={option}
                    className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-right text-sm transition ${
                      option === value ? "bg-emerald-50 text-emerald-700" : "text-slate-700 hover:bg-slate-50"
                    }`}
                    onClick={() => {
                      onChange(option);
                      setIsOpen(false);
                ...
                <div className="px-3 py-4 text-sm text-slate-500">{t("form.noLocationResults")}</div>
              )}
            </div>
          </div>
        ) : null}
...
function validateField(field: FormField, form: CreateEmployerJobPayload): string | undefined {
  switch (field) {
    ...
    case "applicationDeadline":
      ...
      return Number.isNaN(Date.parse(form.applicationDeadline)) ? t("validation.requiredField") : undefined;
    case "salaryMin":
...
export function EmployerJobForm({ onSubmit, submitLabel }: EmployerJobFormProps) {
  ...
  const updateField = <Key extends FormField>(field: Key, value: CreateEmployerJobPayload[Key]) => {
    ...
    if ((field === "salaryMin" || field === "salaryMax") && (touched.salaryMin || touched.salaryMax)) {
      setErrors((current) => ({
        ...current,
  ...
  const experienceOptions: DropdownOption<ExperienceLevel>[] = experienceLevels.map((experienceLevel) => ({
    ...
    <div className="grid gap-5">
        ...
        <label className="space-y-2 text-sm text-slate-700">
          ...
          <input
            className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${
              errors.title ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
            }`}
            required
            value={form.title}
          ...
          />
          {errors.title ? <span className="text-xs font-medium text-rose-600">{errors.title}</span> : null}
        </label>
        <label className="space-y-2 text-sm text-slate-700">
          <span className="flex items-center font-medium">
          ...
          <input
            className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${
              errors.companyName ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
            }`}
            required
          ...
          />
          {errors.companyName ? <span className="text-xs font-medium text-rose-600">{errors.companyName}</span> : null}
        </label>
      </div>
      ...
      <label className="space-y-2 text-sm text-slate-700">
        ...
        <textarea
          className={`min-h-36 w-full rounded-2xl border px-4 py-3 outline-none transition ${
            errors.description ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
          }`}
          required
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className="space-y-2 text-sm text-slate-700">
          ...
          <input
            className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${
              errors.applicationDeadline ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
            }`}
            required
            type="date"
      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
        <label className="space-y-2 text-sm text-slate-700">
          ...
          <input
            className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${
              errors.salaryMin ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
            }`}
            min={0}
            type="number"
          ...
          />
          {errors.salaryMin ? <span className="text-xs font-medium text-rose-600">{errors.salaryMin}</span> : null}
        </label>
        <label className="space-y-2 text-sm text-slate-700">
          <span className="font-medium">{t("form.salaryMax")}</span>
          <input
            className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${
              errors.salaryMax ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
            }`}
            min={0}
            type="number"
          ...
          />
          {errors.salaryMax ? <span className="text-xs font-medium text-rose-600">{errors.salaryMax}</span> : null}
        </label>
        <label className="space-y-2 text-sm text-slate-700">
          <span className="font-medium">{t("form.vacancies")}</span>
          <input
            className={`w-full rounded-2xl border px-4 py-3 outline-none transition ${
              errors.vacancies ? "border-rose-300 focus:border-rose-500" : "border-slate-200 focus:border-emerald-500"
            }`}
            min={1}
            type="number"
          ...
          />
          {errors.vacancies ? <span className="text-xs font-medium text-rose-600">{errors.vacancies}</span> : null}
        </label>
            ...
            {(["full-time", "part-time"] as JobType[]).map((jobType) => (
              <button
                ...
                className={`flex-1 rounded-2xl border px-4 py-3 text-sm font-semibold transition ${
                  form.type === jobType ? "border-emerald-600 bg-emerald-600 text-white" : "border-transparent bg-slate-100 text-slate-700"
                }`}
