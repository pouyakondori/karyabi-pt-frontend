        ...
        return (
          <article
            ...
            className={`rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition ${
              onJobClick ? "cursor-pointer hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md" : ""
            }`}
              ...
              <div className="mt-4 grid gap-3 text-sm text-slate-600 sm:grid-cols-3">
                ...
                {job.applicationDeadline ? (
                  <div className="rounded-2xl bg-sky-50 px-4 py-3 text-sky-900">
                    ...
                    <div className="mt-1 font-bold">{new Date(job.applicationDeadline).toLocaleDateString("fa-IR")}</div>
                  </div>
                ) : null}
              </div>
            ) : null}
