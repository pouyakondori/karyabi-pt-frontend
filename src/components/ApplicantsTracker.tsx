...
export function ApplicantsTracker({
  jobs,
  candidates,
  selectedJobId,
  onSelectJob,
}: ApplicantsTrackerProps) {
  return (
    <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
      <div className="space-y-3">
        {jobs.length === 0 ? (
          <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">
            {t('employer.emptyJobs')}
          </div>
        ) : null}
        {jobs.map(job => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-500">
                {job.location ? (
                  <span className="rounded-full bg-white/80 px-2 py-1">{job.location}</span>
                ) : null}
                {salary ? (
                  <span className="rounded-full bg-white/80 px-2 py-1">{salary}</span>
                ) : null}
              </div>
