        ...
        <div className="grid gap-6 xl:grid-cols-3">
          <SectionCard title={t("admin.employersTitle")}>
            <div className="space-y-4">
              {employers.length === 0 ? <div className="text-sm text-slate-500">{t("admin.emptyEmployers")}</div> : null}
              {employers.map((employer) => (
                <div key={employer.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="font-bold text-slate-900">{employer.email}</div>
                      <div className="mt-1 text-xs text-slate-500">{t("admin.createdAt")}: {formatDate(employer.createdAt)}</div>
                      <div className="mt-2 text-xs text-slate-500">{t("admin.jobsCount")}: {employer._count?.employerJobs ?? 0}</div>
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusChip(employer.isSuspended)}`}>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      ...
                      onClick={() => {
                        void api.suspendAdminUser(employer.id, !employer.isSuspended).then(loadOverview);
                      }}
                      type="button"
                    >
          ...
          <SectionCard title={t("admin.jobSeekersTitle")}>
            <div className="space-y-4">
              {jobSeekers.length === 0 ? <div className="text-sm text-slate-500">{t("admin.emptyJobSeekers")}</div> : null}
              {jobSeekers.map((jobSeeker) => (
                <div key={jobSeeker.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="font-bold text-slate-900">{jobSeeker.seekerProfile?.fullName || jobSeeker.email}</div>
                      <div className="mt-1 text-xs text-slate-500">{jobSeeker.email}</div>
                      <div className="mt-1 text-xs text-slate-500">{t("admin.createdAt")}: {formatDate(jobSeeker.createdAt)}</div>
                      <div className="mt-2 text-xs text-slate-500">{t("admin.applicationsCount")}: {jobSeeker._count?.applications ?? 0}</div>
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${statusChip(jobSeeker.isSuspended)}`}>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      className="rounded-2xl bg-amber-500 px-3 py-2 text-xs font-semibold text-white"
                      onClick={() => {
                        void api.suspendAdminUser(jobSeeker.id, !jobSeeker.isSuspended).then(loadOverview);
                      }}
                      type="button"
                    >
          ...
          <SectionCard title={t("admin.jobVacanciesTitle")}>
            <div className="space-y-4">
              {jobs.length === 0 ? <div className="text-sm text-slate-500">{t("admin.emptyJobsList")}</div> : null}
              {jobs.map((job) => (
