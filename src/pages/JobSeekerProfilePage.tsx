...
export function JobSeekerProfilePage() {
  ...
  const expertise = useMemo(() => profile?.generalExpertise?.join("، ") ?? "—", [profile?.generalExpertise]);
        ...
        <>
          <SectionCard title={t("seeker.profileOverviewTitle")}>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              ...
              <ProfileInfoItem label={t("form.phoneNumber")} value={formatText(profile?.phoneNumber)} />
              <ProfileInfoItem
              ...
              <ProfileInfoItem label={t("form.resumeUrl")} value={formatText(profile?.resumeUrl)} />
              <ProfileInfoItem label={t("form.linkedinUrl")} value={formatText(profile?.linkedinUrl)} />
            </div>
          </SectionCard>

          <SectionCard title={t("seeker.profileQualificationsTitle")}>
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              <ProfileInfoItem label={t("form.hasResidencePermit")} value={formatBoolean(profile?.hasResidencePermit)} />
              <ProfileInfoItem label={t("form.hasWorkPermit")} value={formatBoolean(profile?.hasWorkPermit)} />
              <ProfileInfoItem label={t("form.canRideBike")} value={formatBoolean(profile?.canRideBike)} />
              <ProfileInfoItem
                label={t("form.hasPortugueseDrivingLicense")}
                value={formatBoolean(profile?.hasPortugueseDrivingLicense)}
              />
              <ProfileInfoItem label={t("form.hasUberExperience")} value={formatBoolean(profile?.hasUberExperience)} />
