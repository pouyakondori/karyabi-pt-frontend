    ...
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-700">
            <div className="font-semibold text-slate-900">
              {profile?.fullName || t('seeker.profileIncompleteTitle')}
            </div>
            <div className="mt-2 text-slate-600">
              {profile
                ? t('seeker.profileAvailableDescription')
                : t('seeker.profileIncompleteDescription')}
            </div>
          </div>
          <Link
      ...
      <SectionCard title={t('seeker.recommendedTitle')}>
        <JobList
          ...
          onApply={jobId => void api.applyToJob(jobId)}
        />
      </SectionCard>
    </div>
  )
