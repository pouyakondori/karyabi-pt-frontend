  ...
  const qualificationItems = useMemo(
    () => [
      { label: t('form.hasResidencePermit'), value: formatValue(profile?.hasResidencePermit) },
      {
        ...
        value: profile?.residencyExpirationDate
          ? new Date(profile.residencyExpirationDate).toLocaleDateString('fa-IR')
          : '—',
      },
    ...
    return (
      <SectionCard title={t('employer.candidateNotFoundTitle')}>
        <div className="space-y-4 text-sm text-slate-600">
          ...
          <Link
            className="inline-flex rounded-2xl bg-slate-900 px-4 py-2 font-semibold text-white"
            to={jobOfferPath}
          >
    ...
    <>
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Link
            className="inline-flex w-fit rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm"
            to={jobOfferPath}
          >
            {t('employer.backToJobOffer')}
          </Link>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
            <div className="space-y-4">
              ...
              <p className="text-sm leading-8 text-slate-200">
                {t('employer.candidateProfileTitle')}
              </p>
              {profile?.generalExpertise?.length ? (
                <div className="flex flex-wrap gap-2">
                  {profile.generalExpertise.map(item => (
                    <span
                      key={item}
                      className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs text-emerald-100"
                    >
                      {item}
                    </span>
                  ))}
            ...
            <div className="rounded-[1.5rem] border border-white/10 bg-white/10 p-5 backdrop-blur">
              <div className="text-sm font-bold text-white">
                {t('employer.candidateStatusTitle')}
              </div>
              <div className="mt-4 grid gap-3 text-sm text-slate-200">
                <div>
                  <div className="text-xs text-slate-300">{t('labels.appliedAt')}</div>
                  <div className="mt-1 font-semibold">
                    {new Date(application.appliedAt).toLocaleDateString('fa-IR')}
                  </div>
                </div>
                <div>
                ...
                {application.reviewedAt ? (
                  <div>
                    ...
                    <div className="mt-1 font-semibold">
                      {new Date(application.reviewedAt).toLocaleDateString('fa-IR')}
                    </div>
                  </div>
                ) : null}
        ...
        <SectionCard title={t('employer.candidateLinksTitle')}>
          ...
          {resumeError ? (
            <div className="mt-4 text-sm font-medium text-rose-600">{resumeError}</div>
          ) : null}
        </SectionCard>
      </div>
      ...
      {activeModal === 'reject' ? (
        ...
        >
          <div className="mt-4 space-y-2">
            ...
            />
            {reasonError ? (
              <div className="text-xs font-medium text-rose-600">{reasonError}</div>
            ) : null}
          </div>
        </Modal>
      ) : null}
